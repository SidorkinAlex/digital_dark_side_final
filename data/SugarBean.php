...
class SugarBean
{
    ...
    public function save($check_notify = false)
    {
        ...
        //исключаем запросы создания связей
        if ($_REQUEST['action'] != 'Save2') {
            $seedModule = BeanFactory::getBean($this->module_dir, $this->id);
            $arr = [];
            $request_name = [];
            foreach ($this->field_name_map as $k => $v) {
                if (isset($v['rel_link']) && !empty($v['rel_link'])) {
                    $this->load_relationship($v['rel_link']);
                    $arr[$v['name']] = $this->{$v['rel_link']}->get();
                }
            }


            foreach ($arr as $k => $v) {
                if (!array_key_exists($k, $_REQUEST)) continue; //HRCRM-945
                $new_values = $_REQUEST[$k];
                $old_values = $v;
                if (empty($new_values) and !empty($old_values)) {
                    ...
                    $diff_add = array_diff($new_values, $old_values);
                    $diff_rm = array_diff($old_values, $new_values);
                }
                $this->load_relationship($this->field_name_map[$k]['rel_link']);
                if (!empty($diff_add)) {
                    foreach ($diff_add as $value) {
                        if (!empty($value)) {
                            $this->{$this->field_name_map[$k]['rel_link']}->add($value);
                        }
                    }
                }
                if (!empty($diff_rm)) {
                    foreach ($diff_rm as $value) {
                        if (!empty($value)) {
                            $this->{$this->field_name_map[$k]['rel_link']}->delete($this->id, $value);
                        }
                    }
                }
            }
