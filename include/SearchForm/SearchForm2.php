                        ...
                        switch (strtolower($operator)) {
                            case 'subquery':
                                ...
                                if (is_array($sq)) {
                                    ...
                                    if (array_key_exists('no_quotes', $parms) && $parms['no_quotes'] == true) {
                                        //кастомная логика для subquery, чтоб не экранировались одинарные кавычки в SQL (HRCRM-743)
                                        $stringFormatParams = array(0 => $field_value, 1 => $GLOBALS['current_user']->id);
                                        $where .= "{$db_field} $in (" . custom_string_format($parms['subquery'], $stringFormatParams) . ")";
                                    } else {
                                        $stringFormatParams = array(0 => $field_value, 1 => $GLOBALS['current_user']->id);
